...
{
public:
    char Title[100], Author[50], Book_ID[20], Stud_ID[20];
    int  p,q,B;
    Lib()
    {
        strcpy_s(Title, "NO Book Name");
        strcpy_s(Author, "No Author Name");
        strcpy_s(Book_ID, "No Book ID");
        q = 0;
        B = 0;
        p = 0;
    }
...
void Lib::modify()
{
    ...
    if (i == 1)
    {
        ...
        if (i == 1)
        {
            ...
            while (!intf.eof())
            {
                for (i = 0;  Title[i] != '\0' && st1[i] != '\0' && (st1[i] == Title[i] || st1[i] == Title[i] + 32); i++);
                if (Title[i] == '\0' && st1[i] == '\0')
                {
                    cont++;
                    get_data();
...
void Lib::see(int x)
{
    ...
    if (i == 1)
    {
        ...
        while (!intf.eof())
        {
            for (i = 0; Title[i] != '\0' && ch[i] != '\0' && (ch[i] == Title[i] || ch[i] == Title[i] + 32); i++);
            if (Title[i] == '\0' && ch[i] == '\0')
            {
                cout << "\n\t\tBook Found :-\n";
                show(x);
    ...
    else if (i == 2)
    {
        ...
        while (!intf.eof())
        {
            for (i = 0;  Book_ID[i] != '\0' && ch[i] != '\0' && ch[i] == Book_ID[i]; i++);
...
void Lib::issue()
{
    ...
    else if (i == 4)
    {
        ...
        while (!intf.eof())
        {
            ...
            if (Book_ID[i] == '\0' && Stud_ID[j] == '\0' && st[j] == '\0' && st1[i] == '\0')
            {
                d = q;
                m = B;
                y = p;
    ...
    else if (i == 5)
    {
        ...
        while (!intf.eof())
        {
            for (i = 0; Book_ID[i] != '\0' && st1[i] != '\0' && st1[i] == Book_ID[i]; i++);
            for (j = 0; Stud_ID[j] != '\0' && st[j] != '\0' && st[j] == Stud_ID[j]; j++);
            if (Book_ID[i] == '\0' && Stud_ID[j] == '\0' && st[j] == '\0' && st1[i] == '\0' && cont == 0)
            {
                cont++;
                intf.read((char*)this, sizeof(*this));
                fine(q, B, p, d, m, y);
...
void Lib::der(char st[], int b, int x)
{
    ...
    while (!intf.eof())
    {
        for (i = 0; b == B && Book_ID[i] != '\0' && st[i] != '\0' && st[i] == Book_ID[i]; i++);
